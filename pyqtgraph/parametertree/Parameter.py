...
def param( name, type, **opts ):
    """
    Helper function for easier definition Parameter.
    """
    typeId = getattr( type, '__name__', type )
    return dict( opts, name=name, type=typeId )


def group( name, *children, **opts ):
    """
    Helper function for easier definition Parameter-Groups.
    """
...
class Parameter(QtCore.QObject):
    ...
    **Signals:**
    ...
    @classmethod
    def makeGroup( _class, name, *children, **opts ):
        """
        Static method that creates a new Parameter-Group (or subclass) instance using
        class to select the appropriate class.

        All options are passed appropriately to Parameter's __init__ method.
        """
        return _class( **group( name, *children, **opts ) )
